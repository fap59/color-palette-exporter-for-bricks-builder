...
const extractPalette = (node: BaseNode) => {
  ...
  // جستجوی گره‌های متنی در داخل فریم یا گروه
  const textNode = node.findAll(n => (n as BaseNode).type === "TEXT")[0] as TextNode;
  
  // در صورتی که گره متنی یافت شود، متن آن را در raw و name قرار می‌دهیم
  if (textNode) {
    const colorData = {
      raw: textNode.characters,
      id: "sample-id",  // شناسه گره متنی
      name: textNode.characters // یا نام متنی که می‌خواهید
    };
    result.colors.push(colorData);
  }

  // جستجوی رنگ‌ها از داخل گره‌ها
  const colorNodes = node.findAll(n => n.type === "RECTANGLE" || n.type === "ELLIPSE");
